...
CREATE TABLE IF NOT EXISTS `3dword`.`productos` (
  ...
  PRIMARY KEY (`idProducto`, `clientes_Codigo_Cliente`, `remito_id_remito`),
  INDEX `fk_productos_clientes_idx` (`clientes_Codigo_Cliente` ASC) VISIBLE,
  INDEX `fk_productos_remito1_idx` (`remito_id_remito` ASC) VISIBLE,
  CONSTRAINT `fk_productos_clientes`
    FOREIGN KEY (`clientes_Codigo_Cliente`)
    REFERENCES `3dword`.`clientes` (`Codigo_Cliente`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_productos_remito1`
...
CREATE TABLE IF NOT EXISTS `3dword`.`ventas` (
  ...
  `productos_clientes_Codigo_Cliente` INT(11) NOT NULL,
  PRIMARY KEY (`Id_Ventas`, `productos_idProducto`, `productos_clientes_Codigo_Cliente`),
  INDEX `fk_ventas_productos1_idx` (`productos_idProducto` ASC, `productos_clientes_Codigo_Cliente` ASC) VISIBLE,
  CONSTRAINT `fk_ventas_productos1`
    FOREIGN KEY (`productos_idProducto` , `productos_clientes_Codigo_Cliente`)
    REFERENCES `3dword`.`productos` (`idProducto` , `clientes_Codigo_Cliente`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4;
